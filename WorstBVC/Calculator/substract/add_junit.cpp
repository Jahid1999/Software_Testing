...
int main(){
	...
	//cout<<id<<" calling"<<endl; 
		...
		while(ifile>>id>>ch>>a>>ch>>b>>ch){
			ofile<<"@Test\n";
			ofile<<"public void substractTestWorst"<<id<<" () {"<<endl;
			ofile<<"\tCaculator cal = new Caculator();"<<endl;
			ofile<<"\tint result = cal.substract("<<a<<","<<b<<");"<<endl;
			ofile<<"\tint expected = "<<substract(a,b)<<";"<<endl;
			ofile<<"\tassertEquals(expected,result);"<<endl;
			ofile<<"}"<<endl;
			
			ofileresult<<id<<","<<a<<","<<b<<","<<substract(a,b)<<","<<substract(a,b)<<","<<"Success"<<endl;
